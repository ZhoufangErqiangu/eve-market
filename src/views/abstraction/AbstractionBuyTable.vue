<template>
  <el-table v-loading="loading" :data="dd" height="100%">
    ...
    <el-table-column :label="$t('Ore price')" prop="priceOre"
      :formatter="(row: TableOrder) => priceFormatter(row.priceOre)" :sortable="true" />
    <el-table-column :label="$t('Mineral price')" prop="priceMineral"
      :formatter="(row: TableOrder) => priceFormatter(row.priceMineral)" :sortable="true" />
    <el-table-column :label="$t('Ratio')" prop="ratio" :formatter="(row: TableOrder) => percentFormatter(row.ratio)"
      :sortable="true" />
  </el-table>
</template>
...
const data = ref<Array<TableOrder>>([]);
async function initData() {
  loading.value = true;
  try {
    const rr: Array<TableOrder> = [];
...
watch(() => dataStore.typePortions, initData, { immediate: true });
