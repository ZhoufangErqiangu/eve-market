    ...
    <el-col :span="12">
      <el-form :model="form" :rules="rules" label-width="180px">
        <el-form-item :label="$t('Missile damage')" prop="damage">
          <el-input-number v-model="form.damage" :min="0" :step="0.01" />
        </el-form-item>
        ...
        <el-form-item :label="$t('Explosion velocity')" prop="explosionVelocity">
          <el-input-number v-model="form.explosionVelocity" :min="1" :step="0.01" />
        </el-form-item>
        <el-divider />
        ...
        <el-form-item :label="$t('Target velocity')" prop="velocity">
          <el-input-number v-model="form.velocity" :min="1" :step="0.01" />
        </el-form-item>
        <el-divider />
        <el-form-item>
...
import { ElForm, ElFormItem, ElInputNumber, type FormRules, ElDivider, ElRow, ElCol, ElTable, ElTableColumn, ElButton } from "element-plus";
...
const result = computed(() => {
  const { damage, fireInterval, explosionRadius, explosionVelocity, velocity, signalRadius } = form.value;
  if (damage && fireInterval && explosionRadius && explosionVelocity && velocity && signalRadius) {
    // on board
    ...
    const magnification = Math.max(
      ...
      Math.min(
        ...
        (signalRadius * explosionVelocity) / (explosionRadius * velocity),
      ));
